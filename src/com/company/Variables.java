...
public class Variables<arrayInt> {
    ...
    double numDouble = 109.015; // диапазон значений: ОТ 4.9е-324 ДО 1.8е+308
    ...
    */
    ...
    // 1)если оба типа данных совместимы, их преобразование будет выполнено
    // 2)длина целевого типа больше длины исходного типа.

    long convertIntToLong = numInt; //значение типа int всегда можно присвоить переменной типа long.

    /*---Приведение несовместимых типов---*/

    //Приведение - это всего лишь явное преобразование типов. т.е мы явно указывем в какой тип переоброзовать

    int convertLongToInt = (int) convertIntToLong; // тип long приводится к типу int

    short convertIntToShort = (short) convertLongToInt; // тип int приводится к типу short

    byte convertShortToByte = (byte) convertIntToShort; // тип short приводится к типу byte

    int convertIntToDouble = (int) numDouble; //преобразовывая это число 109.015 в int дробная часть 0.015 усекается
    ...
    В языке Java допускается создание массивов любого типа
    и разной размерности. Доступ к конкретному элементу массива осуществляется
    по его индексу. Массивы предоставляют удобный способ группирования связанной вместе информации. */

    /*---Одномерные массивы---*/

    /* К конкретному элементу массива можно обращаться, указывая его индекс в квадратных
    скобках. Индексы массива начинаются с нуля. */

    // Общая форма объявления (тип имя_переменной[];)

    int arrayInt[];

    // Общая форма операции new применительна (переменная_массива = new тип [размер];)

    {
        arrayInt = new int[12];  // все элементы данного массива будут инициализированы нулевыми значениями
        arrayInt[0] = 100_000_100; // присваивание нуливому элементу значение 100_000_100
    }
    ...
    /* При объявлении переменной многомерного массива для указания каждого дополнительного
    индекса используется отдельный ряд квадратных скобок. */

    int arrayIntTwoDimensional[][] = new int[4][5];

    //Для создания многомерных массивов используются дополнительные скобки:

    int[][] arrayIntTwoDimensional2 = { { 1, 2, 3 }, { 4, 5, 6 }, { 7, 8, 9 } };

    /*---Альтернативный синтаксис объявления массивов---*/

    //Для объявления массивов можно использовать и вторую, приведенную ниже форму (тип[] имя_переменной)
    //В этой форме квадратные скобки следуют за спецификатором типа, а не за именем переменной массива.

    // Например, следующие два объявления равнозначны:
    ...
    /* char arrayCharTwoDimensional[][] = new char[3][4]; */
    char[][] arrayCharTwoDimensional = new char[3][4];

    //Эта форма объявления массивов удобна для одновременного объявления
    //нескольких массивов. Например, в следующем объявлении:

    /* int array1[], array2[], array3[]; //создать три массива */
    int[] array1, array2, array3; // создать три массива
